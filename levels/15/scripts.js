...
arrowUp.addEventListener('click', () => {
  ...
  if (section === 'film') {
    ...
    if (['vocabulary', 'practice', 'props'].includes(section)) {
      if (sec.index > 0) {
        sec.index--;
        showCard(section);
...
const menuProps = document.getElementById('menuProps');
...
menuProps.addEventListener('click', (e) => {
  e.preventDefault();
  showSection('props');
});
...
function showSection(sectionToShow) {
  const sections = ['filmContent', 'vocabularyContent', 'practiceContent', 'colorChartContent', 'whiteboardContent', 'propsContent', 'studentContent'];
  ...
  if (sectionToShow === 'colorChart' || sectionToShow === 'student' || sectionToShow === 'whiteboard' || sectionToShow === 'practice') {
...
const sectionCards = {
  ...
  props: {
    container: document.getElementById('propsContent'),
    get cards() {
      return document.querySelectorAll('#propsContent .vocab-card');
    },
    index: 0
  },
  student: {
...
function generateDotsFor(section) {
  ...
  if (section === 'student'|| section === 'practice') {
